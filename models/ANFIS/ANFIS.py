...
class ANFIS(AbstractANFIS):
    """Traditional ANFIS implementation that generates its own membership functions"""

    def __init__(self, n_input: int, membfuncs: list, to_device: Optional[str] = None,
                 drop_out_rate: Optional[int] = 0.2):
        super(ANFIS, self).__init__(n_input=n_input, to_device=to_device,
                                    drop_out_rate=drop_out_rate, membfuncs=membfuncs)
    ...
    def forward(self, S_batch: torch.Tensor, X_batch: torch.Tensor) -> torch.Tensor:
        """
        Traditional ANFIS forward pass

        Args:
            S_batch: State variables (same as X_batch for traditional ANFIS)
